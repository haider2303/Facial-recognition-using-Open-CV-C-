...
Mat getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, bool doLeftAndRightSeparately, Rect *storeFaceRect, Point *storeLeftEye, Point *storeRightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
	...
	faceCascade.detectMultiScale(srcImg, faces);
	if (faces.size() > 0)
	{
		faceRect = faces[0];
	}*/
    ...
    if (faceRect.width > 0) 
	{
        ...
        if (leftEye.x >= 0 && rightEye.x >= 0) {
			...
			Mat my_warped = Mat(warped);
            warpAffine(gray, warped, rot_mat, warped.size());

			// Coded by Kashan
			warpAffine(perfect, my_warped, rot_mat, my_warped.size());
			//imshow("My Warped By Kashan", my_warped);
            //imshow("Original Warped By Shervin", warped); 
			//return warped; /// Uncomment this inorder to return a grayscale cropped image without an ellipse filter.
			//return my_warped;
